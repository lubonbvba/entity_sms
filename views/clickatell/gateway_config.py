...
class clickatell_core(models.Model):
    ...
    def send_message(self, sms_gateway_id, from_number, to_number, sms_content, my_model_name, my_record_id, my_field_name):
        ...
        clickatell_url = "http://api.clickatell.com/http/sendmsg?user=" + str(sms_account.clickatell_username) + "&password=" + str(sms_account.clickatell_password) + "&api_id=" + str(sms_account.clickatell_api_id) + "&req_feat=48" + "&from=" + str(self.env.user.partner_id.mobile) + "&to=" + str(format_number) + "&text=" + str(sms_content)
        #pdb.set_trace()
