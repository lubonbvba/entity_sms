...
class EsmsHistory(models.Model):
	...
	verified_number_id = fields.Many2one('esms.verified.numbers', readonly=True, string="Verified number")
	...
	my_date = fields.Datetime(string="Send/Receive Date", readonly=True, help="The date and time the sms is received or sent, as reported by the gateway")
	statusdate =  fields.Datetime(string="Last updated", readonly=True, help="Date and time the last update was received")
	...
	number_retries = fields.Integer(help="Number of times this msg id is received")
	...
	@api.multi
	def process_incoming_message(self, to_number, from_number,msg,msg_id,msg_time,full_text=None):
		new_msg=self.search([('sms_gateway_message_id','=', msg_id)])
		if new_msg:
			for m in new_msg:
				m.number_retries +=1
				m.statusdate = fields.Datetime.now()
			_logger.info("Duplicate message to %s with ID: %s received." % (to_number,msg_id))
			return 200
		else:
			verified_number_id=self.env['esms.verified.numbers'].search([('mobile_number','=', to_number)])
			new_msg= self.create({
				'to_mobile': to_number,
				'from_mobile': from_number,
				'sms_content': msg,
				'sms_gateway_message_id':msg_id,
				'statusdate': fields.Datetime.now(),
				'my_date': msg_time,
				'raw_response': full_text,
				'direction': 'I',
				'verified_number_id': verified_number_id.id,
				'account_id': verified_number_id.account_id.id,
				})
			return 200
	...
	def create(self, values):
		...
		if new_rec.direction == 'I':
			...
			for partner in partners:
				...
				partner==self.account_id.browse(values['account_id']).keep_alive_partner_id
				...
				#pdb.set_trace()
					...
					partner.message_post(body=new_rec.sms_content,
						...
						type = 'comment',
						subtype = "entity_sms.mt_sms_received_from_partner")
			...
			for n in incoming_number:
				n.message_post(body=new_rec.sms_content,
					...
					type = 'comment',
					subtype = "entity_sms.mt_sms_received")
		if new_rec.direction =='O':
			if ('partner_id' in values.keys() and values['partner_id']):
				new_rec.partner_id.message_post(body="From: %s\n\r%s" % (new_rec.from_mobile,new_rec.sms_content),
					...
					type = 'comment',
					subtype = "entity_sms.mt_sms_sent_to_partner"
					)
